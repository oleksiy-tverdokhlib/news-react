...
export const getNews = async (
	...
	try {
		...
		const resonse = await axios.get<NewsApiResponse>(`${BASE_URL}search`, {
			params: {
				apiKey: API_KEY,
				page_number,
				page_size,
...
export const getLatestNews = async (): Promise<NewsApiResponse> => {
	try {
		const resonse = await axios.get<NewsApiResponse>(`${BASE_URL}latest-news`, {
			params: {
				apiKey: API_KEY,
			},
		})
...
export const getCategories = async (): Promise<CategoriesApiResponse> => {
	try {
		const resonse = await axios.get<CategoriesApiResponse>(
